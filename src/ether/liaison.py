...
#     def __init__(self, session_config: EtherSessionConfig = None):
...
#         session_data = ether.session_info
#         if session_data and "session" in session_data:
#             session_config = EtherSessionConfig.model_validate(session_data["network"])
#             self._logger.debug(f"Using network config from session: {session_config}")
#         else:
#             self._logger.debug("No session found, using default session config")
...
#         redis_url = f"redis://{host}:{session_config.redis_port}"
